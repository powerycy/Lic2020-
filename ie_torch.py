            ...
            for _start, predicate1 in zip(*start):
                for _end, predicate2 in zip(*end):
                    if _start <= _end and predicate1 == predicate2:
                        if len(mapping[subject[0]]) > 0 and len(mapping[subject[1]]) > 0 and len(mapping[_start]) > 0 and len(mapping[_end]) > 0:
                            spoes.append(
                                ((mapping[subject[0]][0],
...
class data_generator:
    ...
    def __init__(self, data, batch_size, buffer_size=None):
        self.data = data
...
dg = data_generator(train_data,batch_size)
...
if fp16 == True:
    sub_model.half()
# sub_model.to(device)
f = open(r'res_1.txt','a',encoding='utf8')
sub_model.to(device)
